...
**Stack:** C# on .NET 10 (records, async/await + Task), ASP.NET Core, BouncyCastle.Cryptography (PQC), ASP.NET Core WebSockets middleware.
...
| Concern | Algorithm | Standard | BouncyCastle.Cryptography API |
|---|---|---|---|
| Digital signatures (transactions, blocks, handshake) | ML-DSA-65 (CRYSTALS-Dilithium) | **FIPS 204** | `Org.BouncyCastle.Crypto.Signers.MLDsaSigner` + `MLDsaKeyPairGenerator` (parameter set `ml_dsa_65`) |
| Key encapsulation (P2P session keys) | ML-KEM-768 (CRYSTALS-Kyber) | **FIPS 203** | `Org.BouncyCastle.Crypto.Kems.MLKemEncapsulator` / `MLKemDecapsulator` |
| Hashing (block hash, Merkle root, address derivation) | SHA3-256 | FIPS 202 | `Org.BouncyCastle.Crypto.Digests.Sha3Digest` |
| Symmetric channel | AES-256-GCM | FIPS 197 / SP 800-38D | `System.Security.Cryptography.AesGcm` |

All PQC primitives come from the **`BouncyCastle.Cryptography` 2.6.2** NuGet package — the single crypto dependency, wired as DI singletons at startup. **Do not** hand-roll PQC or depend on OS-provided experimental PQC — the NuGet package keeps the build portable across platforms.

### 1.2 Services

- `PqcSignatureService` — key pair generation, `Sign(byte[])`, `Verify(pubKey, data, sig)`. XML doc comments (`///`) must explain why lattice signatures survive Shor's algorithm while ECDSA does not.
- `PqcKeyExchangeService` — ML-KEM encapsulation/decapsulation + HKDF-style derivation of the AES-256-GCM session key. XML doc comments must explain the QKD analogy: like QKD, each session gets a *fresh, jointly-derived, never-transmitted* key; unlike QKD, security rests on lattice hardness instead of photon physics.
...
- `Block` and `Transaction` are **C# records** (immutable). Block hash = SHA3-256 over `index‖timestamp‖previousHash‖merkleRoot‖validatorId‖nonce‖proof`.
...
- Each process is one node: identity = wallet fingerprint; configuration via the `"LegalChain"` section of `appsettings.json` + `--port` / `--node-name` CLI overrides for the second node.
...
- All request handling and P2P I/O is **fully asynchronous** (`async`/`await` + `Task` on the .NET thread pool — the .NET analogue of Java virtual threads; the P2P client uses `System.Net.WebSockets.ClientWebSocket` asynchronously).
...
- **BFT (emphasized)** — simulated 4-validator PBFT round (pre-prepare/prepare/commit); proof records the ≥2f+1 vote set; tolerates f < n/3 faulty validators. XML doc comments must explain why BFT finality suits permissioned, compliance-oriented ledgers.
...
- Transport: **WebSocket** endpoint `/ws/p2p` served by raw ASP.NET Core `UseWebSockets()` middleware (outbound client via `System.Net.WebSockets.ClientWebSocket`, fully async; no SignalR).
...
- Doc-comment requirement (from CLAUDE.md ROLE 2): explain in XML doc comments how signed-handshake + KEM establishes trust between parties **without revealing full identities** (pseudonymous fingerprints, proof of key possession — a ZKP-flavored property).
...
| NFR | Requirement |
|---|---|
| Code quality | C# records for domain, async/await (`Task`) for all I/O, XML doc comments explaining *why compliant & secure* on every crypto/consensus/P2P class. |
| Testing | xUnit tests (`dotnet test`): chain integrity & tamper detection, ML-DSA sign/verify roundtrip, ML-KEM shared-secret agreement, consensus seal/verify, contract replay. |
