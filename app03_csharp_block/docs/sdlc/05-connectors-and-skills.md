...
|---|---|---|---|
| **P2P node channel** | ASP.NET Core WebSockets middleware (`/ws/p2p`), async `ClientWebSocket` outbound | Full-ledger synchronization between two remote nodes; block/tx gossip | ML-DSA-signed handshake, ML-KEM-768 session encapsulation, AES-256-GCM frames; pseudonymous fingerprints only (GDPR-clean) |
...
| **BouncyCastle PQC provider** | `BouncyCastle.Cryptography` 2.6.2 (NuGet) | ML-DSA (FIPS 204) signatures, ML-KEM (FIPS 203) KEM, SHA3-256 | Wired as DI singletons at startup; the single source of crypto primitives — no hand-rolled crypto |
| **REST API** | ASP.NET Core MVC controllers (JSON), contract v1 (doc 03) | Commands & queries: chain, wallet, NFT, contracts, consensus, P2P control | CORS restricted to SPA origin; input validation on every mutation |
...
|---|---|---|
| **IPFS (real)** | `net-ipfs-http-client` / Helia gateway | NFT metadata pinning & retrieval instead of URI stub |
| **GraphQL** | HotChocolate (ASP.NET Core) | When frontend needs flexible ledger queries (per-address history, contract state slices) |
| **gRPC** | grpc-dotnet + protobuf | >2 nodes or cross-language peers; streaming block gossip at scale |
...
| **ZKP library** | e.g. zk-SNARK toolkit (ingonyama/gnark-style or .NET bindings) | Upgrade ZKP-*style* pseudonymous auth to actual zero-knowledge proofs (voting, consent) |
| **Persistence** | PostgreSQL (Npgsql)/RocksDB snapshotting | Chain survives node restarts; audit-grade retention |
...
| Skill | Scope |
|---|---|
| `quantum-crypto-engineering` | Selecting/wiring NIST PQC primitives via `BouncyCastle.Cryptography`: ML-DSA parameter sets (`MLDsaSigner`/`MLDsaKeyPairGenerator`), ML-KEM via `MLKemEncapsulator`/`MLKemDecapsulator`, HKDF key derivation, GCM nonce discipline; reviewing crypto code for misuse (key reuse, nonce reuse, unauthenticated handshakes). |
| `aspnet-core-ledger-management` | ASP.NET Core (.NET 10) idioms for ledger nodes: records-based domain, async/await concurrency, raw WebSocket middleware endpoints, thread-safe mempool/chain, REST contract v1 conformance, integrity test design. |
...
| `compliance-audit` | Checking GDPR (no personal data on-chain), eIDAS 2.0 (SSI trajectory), MiCA (closed-loop token) claims against the actual code; writing the "why this is compliant" XML doc comments. |
...
```mermaid
flowchart LR
    SPA[React SPA 🇬🇧/🇵🇱] -->|REST v1| API[ASP.NET Core API]
